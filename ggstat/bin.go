...
type Bin struct {
	...
	// 0}. If X is an integer column, this center will be treated
	// as an integer as well.
	// XXX What does center mean for integers? Should an unspecified center yield an autochosen one, or 0?
	//Center float64

	// Breaks is the set of break points to use. If provided,
	// Width and Center are ignored.
	Breaks table.Slice
	...
	g = table.MapTables(g, func(_ table.GroupID, t *table.Table) *table.Table {
		...
		for i := 0; i < nin; i++ {
			...
			bin := sort.Search(nbreaks, func(j int) bool {
				return generic.OrderR(elt, breaks.Index(j)) == -1
			})
